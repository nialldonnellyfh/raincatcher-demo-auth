...
module.exports = function initialiseUserStore(applicationStore) {
  ...
  //Registerig the "User" schema with the data store for mongoose.
  applicationStore.registerSchema('mymongoose', IDENTITY, userSchema);


  //TODO Mongoose Optimisation (lean, limits etc)
  applicationStore.registerSubscriber('mymongoose', generateTopic('create'), function(userToCreate) {
    return this.model.create(userToCreate);
  });

  //TODO Pagination etc for performance.
  applicationStore.registerSubscriber('mymongoose', generateTopic('list'), function() {
    return this.model.find();
  });

  applicationStore.registerSubscriber('mymongoose', generateTopic('search'), function(searchCriteria) {
    //TODO - Generate search criteria / reference mongoose format etc.
    return this.model.find(searchCriteria);
  });

  applicationStore.registerSubscriber('mymongoose', generateTopic('delete'), function(userId) {
    return this.model.remove(userId);
  });


  //TODO error handling etc.
  applicationStore.registerSubscriber('mymongoose', generateTopic('update'), function(userToUpdate) {
